...
const ChatDashboard = () => {
    ...
    useEffect(() => {
        const loadActiveChats = async () => {
            try {
                const response = await fetch('/api/zendesk/chat/chats'); // Actualizada la ruta
                if (!response.ok) {
                    ...
                    <div className="space-y-2">
                        {activeChats.map((chat) => (
                            <div
                                ...
                                className={`p-4 rounded-lg cursor-pointer hover:bg-gray-50 ${selectedChat === chat.id ? 'bg-blue-50' : ''
                                    }`}
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="flex flex-col items-end">
                                        ...
                                        <span className={`text-xs px-2 py-1 rounded ${chat.status === 'online' ? 'bg-green-100 text-green-800' :
